...
void inicio() {
    ...
    while (op_inicial < 1 || op_inicial > 3) {
        printf("\nO que deseja fazer?\n");
        printf(" 1 - SEGUIR EM FRENTE\n");
        printf(" 2 - VOLTAR PARA TRAS\n");
        printf(" 3 - OLHAR EM VOLTA\n");
        printf("---> ");
        scanf("%d", &op_inicial);

        if (op_inicial == 3) {
            printf("\nVoce nota que as arvores a sua volta sao uma especie de pinheiros\n");
            printf("com copas altas, que tampam a chegada da luz solar ate o solo.\n");
            printf("Atras de voce percebe-se um declinio no solo, algo como um penhasco.\n");
            op_inicial = 0;
        } else if (op_inicial < 1 || op_inicial > 3) {
...
void encontro_goblin() {
    ...
    switch (encontro_gb) {
        case 1:
            printf("\nVoce lembra que tem uma faca em seu bolso e se prepara para lutar...\n");
            luta_goblin();
            break;
...
void luta_goblin() {
    ...
    do {
        ...
        if (jogadaATT == 1) {
            printf("\nVoce desfere um ataque contra os goblins que causa 20 pontos de dano...\n");
            goblinsVD -= 20;
        } else if (jogadaATT == 2) {
            playerVD = 0;
        } else {
    ...
    if (playerVD <= 0) {
        ...
        printf("e voce acabou nao conseguindo vencelos, eles acabaram com voce\n");
        printf("e levaram o seu canivete e alguns trocados que voce tinha...\n");
        printf("VOCE MORREU...\n");
    }

    if (goblinsVD <= 0) {
        printf("\nVoce conseguiu eliminar os 3 goblins com sucesso,\n");
        printf("diante de voce eles deixam as sacolas de objetos roubados.\n");
        printf("Ao abrir, voce ve que estao cheias de ouro e joias.\n");
        printf("Satisfeito com sua vitoria, voce decide seguir em frente\n");
        printf("saindo da floresta e chegando ate um vilarejo, onde se acomoda por algum tempo.\n");
